...
const styles: Record<string, React.CSSProperties> = {
  ...
  listArea: { flex: 1, minHeight: 0, overflowY: "auto" },
  list: { display: "flex", flexDirection: "column", gap: "8px" },
  resizeHandle: { position: "absolute", top: 0, right: 0, width: "6px", height: "100%", cursor: "col-resize" },
};
...
export default function Sidebar({
  ...
  return (
    <aside style={{ ...styles.sidebar, width: collapsed ? COLLAPSED_WIDTH : width }}>
      <div style={styles.header}>
        {!collapsed && <h2 style={styles.title}>Chats</h2>}
        <button type="button" style={styles.iconButton} onClick={() => setCollapsed((prev) => !prev)}>
          {collapsed ? "›" : "‹"}
        </button>
      </div>
